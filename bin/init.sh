...
#   - Replace slug from `pt-api` to one of two (first in first used):
#      - user input
...
declare -a files_with_port=(.env Dockerfile README.md src/main/resources/application.yaml charts/pt-api/values.yaml)
declare -a files_with_slug=(build.gradle docker-compose.yml Dockerfile README.md ./infrastructure/main.tf ./src/main/java/uk/gov/hmcts/reform/demo/controllers/RootController.java charts/pt-api/Chart.yaml)

# Replace in CNP file
for i in "Jenkinsfile_template"
do
  perl -i -pe "s/pt/$product_name/g" ${i}
  perl -i -pe "s/demo/$component_name/g" ${i}
done

# Replace image repo
for i in "charts/pt-api/values.yaml"
do
  perl -i -pe "s/pt/$product_name/g" ${i}
  perl -i -pe "s/pt-api/$component_name/g" ${i}
done

# Remove "pt-" prefix from chart name to prepare it for pt-api slug replacement and update maintainer name
for i in "charts/pt-api/Chart.yaml"
do
  perl -i -pe "s/pt-//g" ${i}
  perl -i -pe "s/pt/$product_name/g" ${i}
done
...
git mv charts/pt-api charts/${slug}
